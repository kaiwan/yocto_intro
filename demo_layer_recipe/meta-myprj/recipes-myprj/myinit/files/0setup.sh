...
alias jlog='/bin/journalctl --no-pager -b --all --catalog --no-pager'
# jlogr: current boot only, everything, *reverse* chronological order
alias jlogr='/bin/journalctl --no-pager -b --all --catalog --no-pager --reverse'
# jlogall: *everything*, all time; --merge => _all_ logs merged
alias jlogall='/bin/journalctl --no-pager --all --catalog --merge --no-pager'
# jlogf: *watch* log, 'tail -f' mode
alias jlogf='journalctl --no-pager -f'
# jlogk: only kernel messages, this boot
alias jlogk='journalctl --no-pager -b -k'
